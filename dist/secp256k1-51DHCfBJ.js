...
class HashMD extends xt.Hash {
  constructor(blockLen, outputLen, padOffset, isLE) {
    ...
    this.view = xt.createView(this.buffer);
  }
  update(data) {
    xt.aexists(this);
    data = xt.toBytes(data);
    xt.abytes(data);
    ...
    for (let pos = 0; pos < len; ) {
      ...
      if (take === blockLen) {
        const dataView = xt.createView(data);
  ...
  digestInto(out) {
    ...
    let { pos } = this;
    buffer[pos++] = 128;
    xt.clean(this.buffer.subarray(pos));
...
class SHA256 extends HashMD {
  ...
  process(view, offset) {
    ...
    for (let i = 16; i < 64; i++) {
      ...
      const s0 = xt.rotr(W15, 7) ^ xt.rotr(W15, 18) ^ W15 >>> 3;
      const s1 = xt.rotr(W2, 17) ^ xt.rotr(W2, 19) ^ W2 >>> 10;
      SHA256_W[i] = s1 + SHA256_W[i - 7] + s0 + SHA256_W[i - 16] | 0;
    }
    let { A, B, C, D, E, F, G, H } = this;
    for (let i = 0; i < 64; i++) {
      const sigma1 = xt.rotr(E, 6) ^ xt.rotr(E, 11) ^ xt.rotr(E, 25);
      const T1 = H + sigma1 + Chi(E, F, G) + SHA256_K[i] + SHA256_W[i] | 0;
      const sigma0 = xt.rotr(A, 2) ^ xt.rotr(A, 13) ^ xt.rotr(A, 22);
  ...
  destroy() {
    ...
    xt.clean(this.buffer);
  }
}
const sha256 = /* @__PURE__ */ xt.createHasher(() => new SHA256());
/*! noble-curves - MIT License (c) 2022 Paul Miller (paulmillr.com) */
